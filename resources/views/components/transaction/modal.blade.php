<div x-show="open" x-cloak x-transition @click.self="open = false"
    ...
    <div @click.stop class="relative w-full max-w-xl rounded-lg bg-white shadow-lg p-4 sm:p-6">
        <div class="flex justify-between items-center border-b border-gray-200 pb-3">
            <h3 class="text-lg capitalize font-semibold text-gray-900">
                Transaction {{ $transaction->type }}
            </h3>
            <button @click="open = false" class="text-gray-400 hover:text-gray-900">
        ...
        <form method="POST" action="{{ route('transaction.update', $transaction->id) }}">
            ...
            <div class="mt-4 mb-6">
                <label class="block font-bold text-gray-600 mb-1">TIME & DATE:</label>
                <input type="datetime-local" name="date" :disabled="!edit"
                    value="{{ $transaction->date->format('Y-m-d\TH:i') }}"
                    class="w-full border border-gray-400 text-black rounded-md px-2 py-2" />
            ...
            <div class="mb-6">
                <label class="block font-bold text-gray-600 mb-1">AMOUNT:</label>
                <div class="relative">
                    ...
                    <input type="text" name="amount" :disabled="!edit"
                        ...
                        class="w-full ps-14 p-2.5 border border-gray-400 text-black rounded-lg" />
                ...
                <div class="flex flex-col space-y-1 mb-7">
                    ...
                    <div x-data="{ open: false, selected: @js($defaultSource) }" class="relative">
                        <button type="button" :disabled="true"
                            class="w-full border border-gray-400 rounded-md px-2 py-2 flex justify-between items-center cursor-not-allowed">
            ...
            <div class="mb-6">
                <label class="block font-bold text-gray-600 mb-1">NOTES:</label>
                <textarea name="notes" :disabled="!edit" rows="3"
                    class="w-full border border-gray-400 text-black rounded-md px-2 py-2">{{ old('notes', $transaction->notes) }}</textarea>
